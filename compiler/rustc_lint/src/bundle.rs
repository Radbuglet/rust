...
declare_lint! {
    /// TODO: Document
    pub BUNDLE_INFER_OVERLAP,
    Deny,
    "context borrowed implicitly by the function overlaps with a bundle it consumes",
}
...
declare_lint_pass!(
    BundleLintPass => [BUNDLE_INFER_OVERLAP, UNNECESSARY_BUNDLE_ITEM]
);

impl<'tcx> LateLintPass<'tcx> for BundleLintPass {
    // TODO: move this into `check_ty` to allow users to suppress the lint at the argument level
    fn check_fn(
        &mut self,
        cx: &LateContext<'_>,
        _fn_kind: FnKind<'_>,
        ...
        let mut tainted_lifetimes = None;
        let mut component_borrowed = None;
        ...
        for (i, arg) in sig.inputs().iter().enumerate() {
            ...
            // We use this information for two lints...

            // `bundle_infer_overlap`
            {
                let component_borrowed = component_borrowed.get_or_insert_with(|| {
                    tcx.components_borrowed(def_id)
                });

                let offending = concrete.iter()
                    .copied()
                    .filter(|(_re, did, _ty)| component_borrowed.has(*did))
                    .map(|(_re, _did, ty)| ty)
                    .collect::<Vec<_>>();
                ...
                for ty in offending {
                    cx.emit_span_lint(
                        BUNDLE_INFER_OVERLAP,
                        span,
                        lints::BundleInferOverlap {
                            ty,
                            origin: ty::BorrowOriginsSubdiag::new(tcx, def_id.to_def_id()),
                        },
                    );
                }
            }
            ...
            // `unnecessary_bundle_item`
            {
                ...
                let offending = concrete.iter()
                    .copied()
                    .filter(|(re, _did, _ty)| match re.kind() {
                        RegionKind::ReLateParam(late) => {
...
struct ConcreteItemVisitor<'tcx> {
    concrete: Vec<(Region<'tcx>, DefId, Ty<'tcx>)>,
}

impl<'tcx> ConcreteItemVisitor<'tcx> {
    fn visit(&mut self, ty: Ty<'tcx>) {
        match ty::ReifiedBundleItemSet::decode(ty) {
            ty::ReifiedBundleItemSet::Ref(re, _muta, did) => {
                self.concrete.push((re, did, ty));
            }
