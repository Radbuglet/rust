    ...
    fn visit_terminator_after_primary_effect(
        ...
        match term.kind {
            ...
            | TerminatorKind::CoroutineDrop => {
                ...
                // Returning from a function invalidates all context borrows since users are not
                // allowed to constrain universal lifetimes to borrows of context items.
                self.check_context_ref_invalidate(loc, state, None);
            }
...
impl<'a, 'tcx> MirBorrowckCtxt<'a, '_, 'tcx> {
    ...
    fn iter_live_context_ref_borrows(
        &self,
        state: &BorrowckDomain<'a, 'tcx>,
        mut f: impl FnMut(BorrowIndex, DefId, Mutability),
    ) {
        ...
        for bw_id in state.borrows.iter() {
            ...
            let Some((ctx_id, mutbl)) = bw_place.as_context_borrow(self.body) else {
                continue;
            };

            f(bw_id, ctx_id, mutbl);
        }
    }

    /// Checks a borrow to `ContextRef`, whether it be directly generated by an `Rvalue::ContextRef`
    /// or indirectly generated by a call to a function which produces a `ContextRef` internally.
    fn check_context_ref_borrow(
        &self,
        location: Location,
        state: &BorrowckDomain<'a, 'tcx>,
        ctx_id: DefId,
        ...
        self.iter_live_context_ref_borrows(state, |bw_id, other_ctx_id, other_mutability| {
            ...
            // TODO: produce an actual diagnostic
            eprintln!(
    ...
    fn check_context_ref_invalidate(
        &self,
        location: Location,
        state: &BorrowckDomain<'a, 'tcx>,
        invalidated: Option<&[DefId]>,
    ) {
        self.iter_live_context_ref_borrows(state, |bw_id, ctx_id, _mutbl| {
            // Ensure that the context item has been invalidated.
            if invalidated.is_some_and(|v| !v.contains(&ctx_id)) {
                return;
            }
            ...
            // TODO: produce an actual diagnostic
            println!(
                "leaked borrow starting at {:?} at {location:?}",
                self.borrow_set[bw_id].reserve_location,
            );
        });
    }
    ...
    fn consume_rvalue(
        ...
        match rvalue {
            ...
            &Rvalue::ContextRef(_, ctx_id, mutability) => {
                self.check_context_ref_borrow(location, state, ctx_id, mutability);
            },
